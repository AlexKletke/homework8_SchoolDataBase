...
def import_data(data, table):
    if table == 1:
        ...
        with open('lesson_database.txt', 'r') as read_object:
            lines = read_object.readlines()            
        with open('lesson_database.txt', 'a') as f2:            
            f2.write(f'{len(lines)+1}, {data}')
            f2.write(f"\n")       
    else:
        with open('symbolclass_database.txt', 'r') as read_object:
            lines = read_object.readlines()
        with open('symbolclass_database.txt', 'a') as f3:
            f3.write(f'{len(lines)+1}, {data}')
            f3.write(f"\n")
